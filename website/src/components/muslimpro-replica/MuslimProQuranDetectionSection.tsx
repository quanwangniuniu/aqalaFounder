...
export default function MuslimProQuranDetectionSection() {
  ...
  const runDetect = async () => {
    ...
    setError(null);
    try {
      const resp = await fetch(`/api/quran/detect?q=${encodeURIComponent(cleanedQuery)}`, {
        method: "GET",
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 md:p-10">
          ...
          <p className="text-white/70 mb-6 max-w-3xl">
            Paste a short snippet you heard (Arabic or translated text). Aqala will search for the closest verse matches.
          </p>
          ...
          <div className="mt-6">
            {results.length === 0 ? (
              <p className="text-white/40 text-sm">
                {loading ? "Searching…" : "No results yet. Paste a snippet and click Detect."}
              </p>
            ) : (
