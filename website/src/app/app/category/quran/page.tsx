...
export default function MuslimProQuranCategoryPage() {
  const searchParams = useSearchParams();
  const page = Math.max(1, parseInt(searchParams?.get("page") ?? "1", 10) || 1);
  const quranPosts = getPostsForCategory("quran");
  const totalPages = getTotalPages(quranPosts);
  const currentPage = Math.min(page, totalPages || 1);
  const posts = getPaginatedPosts(quranPosts, currentPage);
    ...
    <>
      ...
      <section className="py-12 md:py-16 bg-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-sm text-gray-500 mb-2">Category: Quran</p>
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Quran</h1>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {totalPages > 1 && (
            <nav className="mt-12 flex flex-wrap items-center justify-center gap-2" aria-label="Blog pagination">
              <Link
                href={`/app/category/quran?page=${totalPages}`}
              ...
              {Array.from({ length: Math.min(3, totalPages) }, (_, i) => totalPages - 2 + i)
                ...
                .map((p) => (
                  <Link
                    key={p}
                    href={p === 1 ? "/app/category/quran" : `/app/category/quran?page=${p}`}
