...
interface PreferencesContextType {
  ...
  getWallpaperStyle: () => { background: string; backgroundImage?: string };
}
...
export const PreferencesProvider: React.FC<PreferencesProviderProps> = ({ children }) => {
  ...
  // Get the CSS style for the current wallpaper
  const getWallpaperStyle = () => {
    const selected = WALLPAPERS.find((w) => w.id === wallpaper) || WALLPAPERS[0];
    
    // For wallpapers with an image, we need to layer the gradient on top
    if (selected.full) {
      return {
        background: `${selected.gradient}, url(${selected.full})`,
        backgroundColor: "#021a12", // Fallback dark color
      };
    }
