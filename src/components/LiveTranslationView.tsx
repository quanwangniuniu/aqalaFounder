...
export default function LiveTranslationView({
  ...
  const translateText = useCallback(async (text: string, fromLang: string, toLang: string): Promise<string> => {
    ...
    try {
      ...
      if ((error as Error).name === "AbortError") {
        return text; // Return original if aborted
      }
  ...
  // Connect to LiveKit for real-time translation + audio
  const {
    connect,
    isConnected,
    hasRemoteAudio,
  ...
  // Connect on mount
  useEffect(() => {
    if (!hasConnectedRef.current && mosqueId && activeTranslatorId) {
      hasConnectedRef.current = true;
      connect();
    }
  }, [mosqueId, activeTranslatorId, connect]);
      ...
      <div className="flex flex-col h-full w-full overflow-hidden">
        ...
        <main className="flex-1 flex flex-col items-center justify-center text-center px-6">
          <div className="relative mb-8">
            <div className="w-20 h-20 rounded-full bg-gradient-to-br from-white/[0.08] to-white/[0.02] flex items-center justify-center">
            ...
            <div className="absolute inset-0 w-20 h-20 rounded-full border border-white/5 animate-ping" style={{ animationDuration: "3s" }} />
          </div>
          
          <p className="text-white/50 text-lg mb-2" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
            Waiting for broadcast...
          </p>
          <p className="text-white/30 text-sm">The session will begin shortly</p>
        </main>
      </div>
    );
  }
