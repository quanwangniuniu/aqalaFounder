        ...
        <View className="relative overflow-hidden px-5 py-8">
              ...
              >
                {isPartner ? (
                  <LinearGradient
                    ...
                    className="flex-row items-center gap-2 px-4 py-2 rounded-xl"
                  >
                    <Ionicons name="add" size={16} color={darkBg} />
                    <Text style={{ color: darkBg }} className="font-medium text-sm">{t("rooms.createRoom")}</Text>
                  </LinearGradient>
                ) : (
                  <View className="flex-row items-center gap-2 px-4 py-2 rounded-xl bg-white/10">
                    <Ionicons name="add" size={16} color="white" />
                    <Text className="text-white font-medium text-sm">{t("rooms.createRoom")}</Text>
                  </View>
                )}
              </TouchableOpacity>
            )}
          ...
          <View className="items-center gap-4 mb-8">
            <View className="flex-row items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10">
              ...
              <Text className="text-xs font-medium text-white/60">
                {totalLiveRooms} {totalLiveRooms === 1 ? t("rooms.roomLiveNow") : t("rooms.roomsLiveNow")}
              </Text>
            </View>
          ...
          <View className="flex-row gap-4 mb-6">
            <View className="flex-1 items-center p-4 rounded-2xl border border-[#D4AF37]/20">
              ...
              <Text className="text-xs text-white/40 mt-1">{t("rooms.partnerBroadcasts")}</Text>
            </View>
            <View className="flex-1 items-center p-4 rounded-2xl border border-emerald-500/20">
              ...
              <Text className="text-xs text-white/40 mt-1">{t("rooms.communitySessions")}</Text>
            </View>
          </View>
        </View>
        ...
        <View className="px-5 pb-12 gap-8" style={{ maxWidth: 600, alignSelf: 'center', width: '100%' }}>
          ...
          {loading && (
            <View className="items-center justify-center py-16">
              ...
              <Text className="text-white/40 text-sm mt-4">{t("rooms.loadingRooms")}</Text>
            </View>
          )}
          ...
          {!loading && livePartnerRooms.length > 0 && (
            <View>
              <View className="flex-row items-center gap-3 mb-5">
                ...
                <View>
                  <Text className="font-semibold text-white">{t("rooms.officialPartners")}</Text>
                  <Text className="text-xs text-white/40">{t("rooms.verifiedBroadcasts")}</Text>
                </View>
              </View>
            ...
            <View>
              <View className="flex-row items-center justify-between mb-5">
                <View className="flex-row items-center gap-3">
                  ...
                  <View>
                    <Text className="font-semibold text-white">{t("rooms.communitySessions")}</Text>
                    <Text className="text-xs text-white/40">{t("rooms.userHosted")}</Text>
                  </View>
                </View>
              </View>
          ...
          {!loading && totalLiveRooms === 0 && (
            <View className="items-center py-16">
              ...
              <Text className="text-lg font-semibold text-white/70 mb-2">{t("rooms.noLive")}</Text>
              <Text className="text-white/40 text-sm mb-6 text-center">{t("rooms.checkBack")}</Text>
              {user && (
                <TouchableOpacity
                  onPress={() => setShowCreateModal(true)}
                  className="px-6 py-3 rounded-xl bg-white/10"
                >
                  <Text className="text-white font-medium">{t("rooms.createRoom")}</Text>
                </TouchableOpacity>
              )}
            </View>
          )}
