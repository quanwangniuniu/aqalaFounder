...
const SHOW_PAST_TRANSLATIONS = true;
            ...
            {SHOW_PAST_TRANSLATIONS && user && (
              <View className="rounded-2xl bg-white/5 border border-white/10 p-4 mb-4">
                ...
                >
                  ...
                  <Ionicons
                    name={
                      pastTranslationsCollapsed ? "chevron-down" : "chevron-up"
                    }
                    size={18}
                    color="rgba(255,255,255,0.6)"
                  />
                  ...
                  (pastTranslationsLoading ? (
                    ...
                    <View className="mt-2 gap-2">
                      {pastTranslations.slice(0, 3).map((pt) => (
                        <Link
                          key={pt.id}
                          href={`/past-translation/${pt.id}` as never}
                          asChild
                        >
                          <TouchableOpacity
                          ...
                          >
                            ...
                            <View className="flex-1 min-w-0">
                              <Text
                                ...
                                numberOfLines={2}
                              >
                                {(pt.translatedParagraphs[0] || pt.sourceText || "").trim()
                                  ? (pt.translatedParagraphs[0] || pt.sourceText || "").trim().length > 80
                                    ? (pt.translatedParagraphs[0] || pt.sourceText || "").trim().slice(0, 80) +
                                      "..."
                                    : (pt.translatedParagraphs[0] || pt.sourceText || "").trim()
                      ...
                      {pastTranslations.length > 3 ? (
                        <View className="py-2 flex-row items-center justify-center">
                          <Text className="text-xs text-white/40">
                            {t("listen.pastTranslationsCount").replace(
                              "{count}",
                              String(pastTranslations.length),
                            )}
                          </Text>
                        </View>
                      ) : null}
